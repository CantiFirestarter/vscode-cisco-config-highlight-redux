...
function applyTokenColors(context: vscode.ExtensionContext): void {
  ...
  // Load default token rules from file if enabled
  const defaultRules = applyAll ? loadDefaultTokenRules(context) : [];

  // Build rules from configuration overrides
  const rules = buildTokenRulesFromConfig(configColors);

  // Merge rules (deduplicate by scope)
  const getScopeKey = (scope: string | string[]): string =>
    Array.isArray(scope) ? scope.join('|') : scope;

  const existingRulesMap = new Map<string, TextMateRule>(
    currentCustomizations.textMateRules.map((rule: TextMateRule) => [
      getScopeKey(rule.scope),
      rule,
    ]),
  );

  // Apply default file rules first (may override existing by scope)
  defaultRules.forEach((rule: TextMateRule) => {
    existingRulesMap.set(getScopeKey(rule.scope), rule);
  });

  rules.forEach((rule: TextMateRule) => {
    existingRulesMap.set(getScopeKey(rule.scope), rule);
  });
...
function buildTokenRulesFromConfig(
  ...
  // Configuration mappings for all token scopes
  const scopeMappings: ScopeMapping[] = [
    ...
    { configKey: 'keyword.status.up', scope: 'keyword.other.config-keyword.status.up' },
    {
    ...
    { configKey: 'keyword.match.any', scope: 'keyword.other.config-keyword.match.any' },
    { configKey: 'keyword.match.all', scope: 'keyword.other.config-keyword.match.all' },
    { configKey: 'keyword.in-out.in', scope: 'keyword.other.config-keyword.in-out.in' },
    {
    ...
    { configKey: 'keyword.shutdown', scope: 'keyword.other.config-keyword.shutdown' },
    ...
    // Interfaces
    { configKey: 'interface.ethernet', scope: 'entity.name.class.interface.ethernet' },
    { configKey: 'interface.wireless', scope: 'entity.name.class.interface.wireless' },
    { configKey: 'interface.loopback', scope: 'entity.name.class.interface.loopback' },
    {
      configKey: 'interface.portchannel',
      scope: 'entity.name.class.interface.portchannel',
    },
    { configKey: 'interface.tunnel', scope: 'entity.name.class.interface.tunnel' },
    ...
    { configKey: 'interface.serial', scope: 'entity.name.class.interface.serial' },
    { configKey: 'interface.cellular', scope: 'entity.name.class.interface.cellular' },
    {
      configKey: 'interface.virtual-template',
      scope: 'entity.name.class.interface.virtual-template',
    },
    { configKey: 'interface.dialer', scope: 'entity.name.class.interface.dialer' },
    ...
    { configKey: 'config-param.second', scope: 'punctuation.config-param.second' },
    ...
    { configKey: 'config-string.name', scope: 'entity.name.tag.config-string.name' },
    {
    ...
    { configKey: 'bgp.peer-policy.name', scope: 'entity.name.tag.bgp.peer-policy.name' },
    { configKey: 'bgp.peer-group.name', scope: 'entity.name.tag.bgp.peer-group.name' },
    ...
    { configKey: 'group.class-map.name', scope: 'entity.name.tag.group.class-map.name' },
    { configKey: 'group.route-map.name', scope: 'entity.name.tag.group.route-map.name' },
    {
    ...
    { configKey: 'group.community.name', scope: 'entity.name.tag.group.community.name' },

    // ACL
    { configKey: 'acl.access-list.name', scope: 'entity.name.tag.acl.access-list.name' },
    {
    ...
    { configKey: 'crypto.keyring.name', scope: 'entity.name.tag.crypto.keyring.name' },
